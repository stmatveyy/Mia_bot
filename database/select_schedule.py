...
async def get_todays_schedule(database: Database) -> str | None:
    '''Возвращает форматированное текущее расписание'''
    int_to_date = {1: 'mon',
                   2: 'tue',
                   3: 'wen',
                   4: 'thu',
    ...
    if curr_weekday == 'sat' or curr_weekday == 'sun':
        ...
        data = await database.view(query=f'''SELECT {curr_weekday} FROM "{week_to_select}"''')
